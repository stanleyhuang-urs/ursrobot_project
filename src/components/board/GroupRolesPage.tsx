    ...
    <div className="space-y-6">
      {boards.map((board) => (
        <section key={board.id}>
          <h2 className="mb-2 text-sm font-semibold text-neutral-800">{board.name}</h2>
          <div className="divide-y divide-neutral-100 rounded-md border border-neutral-200 bg-white">
            {board.groups.map((group) => (
              <div key={group.id} className="flex items-center justify-between px-4 py-2.5">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-md border border-neutral-200 px-2.5 py-1 text-xs text-neutral-600 hover:bg-neutral-50"
                >
                  <Shield size={13} /> 設定角色
                </button>
              </div>
            ...
            {board.groups.length === 0 && (
              <p className="px-4 py-3 text-sm text-neutral-400">此看板尚無分組</p>
            )}
          </div>
        </section>
      ))}
