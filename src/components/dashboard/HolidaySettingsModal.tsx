  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange} title="國定假日設定">
      ...
      <div className="mb-3 max-h-64 space-y-1 overflow-y-auto">
        {holidays.length === 0 ? (
          <p className="text-sm text-neutral-400">尚未設定任何國定假日。</p>
        ) : (
          holidays.map((h) => (
            <div
              key={h.id}
              className="flex items-center justify-between gap-2 rounded-md border border-neutral-200 px-2 py-1.5 text-sm"
            >
      ...
      <div className="flex items-center gap-2 border-t border-neutral-100 pt-3">
        <input
          ...
          className="rounded-md border border-neutral-300 px-2 py-1.5 text-sm outline-none focus:border-blue-500"
        />
        <input
          value={name}
          ...
          maxLength={30}
          className="min-w-0 flex-1 rounded-md border border-neutral-300 px-2 py-1.5 text-sm outline-none focus:border-blue-500"
        />
        <button
          type="button"
          disabled={submitting}
